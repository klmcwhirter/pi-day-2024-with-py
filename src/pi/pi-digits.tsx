...
export const PiDigitsView: Component = (props) => {
  ...
  return (
    <div class='table h-full border-collapse align-middle'>
        ...
        {(row) => (
          <div class='table-row'>
            <For each={row}>
              {(col) => (
                <span
                  class={`m-1 table-cell aspect-square h-auto w-6 rounded-md border text-center align-middle text-sm ${pallette[col]}`}
                >
                  {col}
                </span>
              )}
...
export const PiDigitsHistoView: Component = (props) => {
  const piState = usePiState();
  const histo = piState.piAdapter().histogram();

  return (
    <div>
      <div class='p-10 text-center text-blue-800'>
        <p class='text-2xl font-semibold'>
          Number of times each digit appears in Pi
        </p>
        <p class='text-lg text-blue-400'>first 1024 digits</p>
      </div>
